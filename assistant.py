...
# Context to store the conversation and the last product queried
conversation_context = {"last_product": None, "messages": []}

# Function to get complete product information with a more conversational message
...
# Function to check stock using the last product in the context
def check_stock_func(product_name=None):
    ...
    if isinstance(stock_result, dict):
        ...
        status = stock_result.get("status", "Status not available")
        
        if available_quantity > 0:
            return f"Yes! The product '{product_name}' is in stock with {available_quantity} units available. Would you like to add it to your cart?"
        else:
...
# Assistant functions configuration
...
# Processes the assistant's response and calls the corresponding function
def process_assistant_response(response):
    ...
    return response['choices'][0]['message'].get('content', "I couldn't find a response. Can I help you with something else?")

# Main function to query the assistant
def query_assistant(user_message):
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[{"role": "system", "content": "I am a shopping assistant that helps you with product information, stock availability, and more."}] + conversation_context["messages"],
        ...
        print(f"Error in OpenAI API: {api_error}")
        return f"There was a problem connecting to the service. Please try again later. Error: {api_error}"
